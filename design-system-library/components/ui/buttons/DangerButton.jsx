...
/**
 * DangerButton Component (Corellux OS style)
 * Botão vermelho de risco/exclusão com padrão 3D Keycap.
 *
 * Props:
...
export default function DangerButton({
    ...
    return (
        <button
            ...
            disabled={isDisabled}
            className={`ds-btn ds-btn-danger ${fullWidth ? 'ds-btn-full' : ''} ${loading ? 'ds-btn-loading' : ''} ${className}`}
            style={style}
            aria-busy={loading}
            {...props}
        >
            {loading ? (
                <>
                    ...
                    <span className="ds-btn-text">Excluindo...</span>
                </>
            ) : (
                <>
